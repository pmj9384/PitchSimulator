...
// PlayerTableParser 검증. ①정상 매핑 ②실제 Resources CSV 전건 대조(역할 8종·총점 300) ③헤더 불일치 ④빈 텍스트
// ⑤깨진 숫자(행 번호) ⑥중복 roleId(대소문자 무시) ⑦열 부족 ⑧범위 검증 경계값 ⑨총점 불일치
public class PlayerTableParserTests
{
    private const string ValidHeader =
        "roleId,speed,stamina,pass,shot,tackle,positioning,pushUp,pressRange,shotBias,passLength,width,lineHeight,displayName,description,icon";

    private const string StRow = "ST,60,45,40,90,20,45,25,8,0.3,15,10,0,스트라이커,,";
    private const string GkRow = "GK,30,40,40,10,60,120,0,3,0.9,25,0,0,골키퍼,,";
    ...
    public void 숫자가_깨진_행은_행번호를_알려준다()
    {
        string csv = ValidHeader + "\n" + StRow + "\n" +
                     "GK,30,40,40,abc,60,120,0,3,0.9,25,0,0,골키퍼,,\n";   // 3행 shot 깨짐
    ...
    {
        string csv = ValidHeader + "\n" + StRow + "\n" +
                     "st,60,45,40,90,20,45,25,8,0.3,15,10,0,,,\n";   // 3행 = 2행과 대소문자만 다른 중복
    ...
    [TestCase("ST,0,105,40,90,20,45,25,8,0.3,15,10,0,,,",   "speed")]        // 스탯 0 (합계는 300)
    [TestCase("ST,60,45,40,90,20,46,25,8,0.3,15,10,0,,,",   "합계")]         // 총점 301
    [TestCase("ST,60,45,40,90,20,45,25,0,0.3,15,10,0,,,",   "pressRange")]   // 압박 거리 0
    [TestCase("ST,60,45,40,90,20,45,25,8,1.5,15,10,0,,,",   "shotBias")]     // 확률 밖
    [TestCase("ST,60,45,40,90,20,45,25,8,0.3,0,10,0,,,",    "passLength")]   // 패스 길이 0
    [TestCase(",60,45,40,90,20,45,25,8,0.3,15,10,0,,,",     "roleId")]       // id 공백
    [TestCase("ST,60,45,40,90,20,45,-1,8,0.3,15,10,0,,,",   "pushUp")]       // 전진 폭 음수
